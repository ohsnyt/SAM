...
final class EvidenceRepository {
    ...
    func linkToPerson(evidenceID: UUID, personID: UUID) throws {
        guard let item = try item(id: evidenceID) else { return }
        if !item.linkedPeople.contains(personID) {
            item.linkedPeople.append(personID)
            try container.mainContext.save()
        }
    }
    ...
    func linkToContext(evidenceID: UUID, contextID: UUID) throws {
        guard let item = try item(id: evidenceID) else { return }
        if !item.linkedContexts.contains(contextID) {
            item.linkedContexts.append(contextID)
            try container.mainContext.save()
        }
    }
    ...
    func acceptSuggestion(evidenceID: UUID, suggestionID: UUID) throws {
        ...
        switch s.target {
        case .person:
            if !item.linkedPeople.contains(s.targetID) {
                item.linkedPeople.append(s.targetID)
            }
        case .context:
            if !item.linkedContexts.contains(s.targetID) {
                item.linkedContexts.append(s.targetID)
            }
        }
    ...
    func removeConfirmedLink(
        ...
        switch target {
        case .person:
            item.linkedPeople.removeAll { $0 == targetID }
        case .context:
            item.linkedContexts.removeAll { $0 == targetID }
        }
