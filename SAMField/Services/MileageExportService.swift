//
//  MileageExportService.swift
//  SAM Field
//
//  Generates IRS-compliant mileage logs as CSV (with UTF-8 BOM for Excel) or PDF.
//  Dollar deduction is calculated at export time using the current IRS rate —
//  not stored per trip — since Sarah applies the rate when filing taxes.
//
//  Commuting trips (home ↔ regular office) are marked non-deductible and
//  excluded from business miles totals.
//

import Foundation
import WebKit

struct MileageExportService {

    // MARK: - CSV

    /// CSV with UTF-8 BOM so Excel on any platform opens it correctly.
    static func csv(for trips: [SamTrip], irsRatePerMile: Double) -> String {
        ...
        // Totals footer — only non-commuting business miles
    ...
    /// Single-page or multi-page PDF mileage log (landscape US Letter).
    /// Uses WKWebView for rendering — must be called on the main actor.
    @MainActor
    ...
    private static func buildHTML(for trips: [SamTrip], irsRatePerMile: Double, periodLabel: String) -> String {
        ...
        <div class="footer">
          Est. deduction: $\(String(format: "%.2f", deduction)) (\(String(format: "%.1f", bizTotal)) mi × $\(String(format: "%.2f", irsRatePerMile))/mi) — Verify with your tax advisor. Generated by SAM Field.
          \(commutingNote)
