...
- **New SwiftData model**: `FormDraft` (`SAMModels-FormDraft.swift`) — `(formKind, subjectID)` keyed JSON snapshot with `payloadVersion`, `displayTitle`, `displaySubtitle`, schema-tolerant decode (`decodeIfPresent` everywhere). Brand-new table, no migration required.
- **`DraftPersistenceService`** — `@MainActor` CRUD using `container.mainContext` (per the single-context rule). Typed `save<Payload: Encodable>` / `load<Payload: Decodable>` for coordinator-backed forms; `saveLegacy` / `loadLegacy` for the existing `DraftStore` `[String:String]` API so its 9 callers (NoteEditor, Compose, GoalEntry, ProductionEntry, ManualTask, ContentDraft, Correction, EventForm) got crash recovery without API changes. Adds `unfinishedDraftDescriptors()`, `runAutoDiscardIfNeeded(ttl:)`, `pendingAutoDiscardNotice()`, `acknowledgePendingAutoDiscardNotice()`, `hasUnfinishedDraft(kind:subjectID:)`.
- **`DraftStore`** — refactored to back its in-memory `[String:String]` cache with disk persistence. Debounced 1.5s flushes; SHA-256-derived stable UUIDs for non-UUID id strings (`"new"`, etc.); cold-start hydration on first load; `flushAllPending()` called from `applicationShouldTerminate` so the last debounce window of typing isn't lost on quit.
- **`DraftBackedFormCoordinator` protocol** — `@MainActor @Observable` form owners adopt this to get the lifecycle: `scheduleDraftFlush()` (debounced via `DraftFlushScheduler`), `flushNow()`, `restoreFromDraft()`, `clearDraft()`, `submitAndClear { try save() }` (only clears on success; re-flushes on throw). Conformers expose `hasUserContent`, optional `draftDisplayTitle` / `draftDisplaySubtitle`.
- **`PostMeetingCaptureCoordinator`** — extracts every editable field from `PostMeetingCaptureView` (17 `@State` declarations gone). Identity from `payload.evidenceID` or hashed `eventTitle|eventDate` for impromptu captures so re-presentations of the same meeting always resolve to the same coordinator. Defensive `sanitizeAgainstPayload(_:)` filters dangling attendee UUIDs on restore. `CapturePayloadSnapshot` Codable mirror lets the Today restore banner reconstruct the full `CapturePayload` without re-querying evidence. `markUserEdit()` stamps `lastEditAt` on every `didSet`; `suppressEditTracking` flag prevents init seeding / draft restore from masquerading as user activity.
- **`AppShellView`** — owns `[UUID: PostMeetingCaptureCoordinator]` dictionary, lazy-creates via `captureCoordinator(for:)`, evicts on submit success via the `onSave` closure. Same coordinator instance reused across re-presentations.
- **`ModalCoordinator` soft-displacement guard** — `requestPresentation` accepts optional `hasUserContentProvider: (() -> Bool)?` and `isStillRelevant: (() -> Bool)?`. When the active sheet's content provider returns `true`, `.replaceLowerPriority` requests below `.critical` priority automatically downgrade to `.queue` — the autonomous sheet waits instead of bumping the user. `.critical` (errors, lock) can still break through. `advanceQueue` skips entries whose `isStillRelevant?() == false` at dequeue time so stale background prompts don't cascade out the moment Sarah finishes typing. Added `resetForTesting()` (DEBUG-only) for test isolation.
- **Idle decay** — `PostMeetingCaptureCoordinator.hasUserContentForDisplacement` gates the block on a 15-minute idle window. After the window elapses, autonomous sheets can take the slot; the on-disk draft keeps the work safe so the only consequence is finding another sheet on top when Sarah returns.
- **`ManagedSheetModifier`** — both variants (`isPresented`, `item`) thread `hasUserContentProvider` and `isStillRelevant` through to `requestPresentation`. Backwards-compatible: existing call sites unchanged.
- **`UnfinishedDraftBanner`** — mounted at the top of `AwarenessView`. Lists up to 3 unfinished drafts with **Resume** (reconstructs `CapturePayload` from `CapturePayloadSnapshot` and posts `.samOpenPostMeetingCapture`) and **Discard** (alert-confirmed delete). Refreshes on `.samFormDraftsDidChange` (posted from `clearDraft()` and the banner's own discard path). Renders an auto-discard notice with plain-English copy when `pendingAutoDiscardNotice().count > 0`; OK button calls `acknowledgePendingAutoDiscardNotice()`.
- **7-day auto-discard** — `DraftPersistenceService.runAutoDiscardIfNeeded()` runs on app launch right after the service is configured. Stale drafts are deleted; the count accumulates in `UserDefaults` (`sam.draftAutoDiscard.pendingCount` + `sam.draftAutoDiscard.lastAt`) until acknowledged. Plain-English copy is mandatory: *"X unfinished notes were cleared. SAM keeps unfinished work for one week. Older notes are cleared automatically so the Today view stays tidy."*
- **Audio retention gate** — `RetentionService.runOnce(...)` consults `DraftPersistenceService.hasUnfinishedDraft(kind: .postMeetingCapture, subjectID:)` keyed by the meeting's `SamEvidenceItem.id` (resolved from `TranscriptSession.calendarEventID` via `sourceUID = "eventkit:<id>"`). If a draft exists, audio purge defers for that session. The draft's own 7-day TTL eventually clears the draft, after which the next retention pass purges the audio — Sarah is never stranded; audio purge is just deferred, not blocked indefinitely.
- **Dismiss button semantics** — `PostMeetingCaptureView` replaced "Cancel" with two distinct actions: **Discard** (red, alert-confirmed, deletes draft) and **Close** (preserves on disk, flushes before dismiss, ESC routes here). **Save** remains the primary action and is unchanged.
- **Phase 5 sheet migrations** — `RelationshipGraphView` and `SphereLensPicker` spheres-management sheets moved to `.managedSheet(priority: .userInitiated, ...)`. The other 10 unmanaged `.sheet` calls in the codebase are intentionally not migrated — they are either nested inside an already-managed parent sheet (where wrapping would break the nesting) or inside an independent `WindowGroup` window (where they can't collide with main-window sheets). See context.md §8.1.
- **47 unique tests** (`DraftPersistenceServiceTests`, `PostMeetingCaptureCoordinatorTests`, `ModalCoordinatorDisplacementTests`, `PostMeetingCaptureCoordinatorIdleDecayTests`, `Phase4DraftBannerTests`) covering: CRUD, schema-tolerant decode, staleness queries, deterministic UUID derivation, cold-start hydration, identity resolution, sheet-flip restore, isolation between meetings, dangling-reference filtering, `hasUserContent`, idle decay, soft-displacement block, `.critical` override, stale-queue dequeue, default-relevance fallback, descriptor surface, auto-discard math, snapshot round-trip, `hasUnfinishedDraft` probe, submit success/failure semantics.

**Why**: Apr 24, Sarah was mid-LinkedIn-import when a post-call capture sheet appeared, SwiftUI tore down the import sheet, and she lost her place. That single incident drove this entire architecture. We had three orthogonal problems that all looked like one: sibling-sheet collision (SwiftUI's "one sheet per window" rule), in-progress form state survival (across displacement, lock, and crash), and timing of autonomous triggers (don't interrupt active work). The Phase 1–5 build addresses all three as one system; removing any single piece reopens the data-loss surface.

**Key design decisions**:
- **State lives in coordinators, not view `@State`**. View unmount destroys `@State`; coordinator instances outlive view mounts. This is the prerequisite for sheet-flip recovery — without it, the rest of the architecture can't help.
- **Draft persistence is a SwiftData table, not in-memory only**. The pre-existing `DraftStore` deferred disk persistence citing encryption concerns; that concern was overcautious given the rest of SAM's sensitive content (notes, summaries, transcripts) is already in SwiftData on disk under the same FileVault + user account protection. On-disk persistence unlocks crash recovery, which in-memory-only could never provide.
- **Submit-success-before-clear, never the other way around**. A failed submit must not destroy the work; the failure path re-flushes the draft so a retry from a fresh sheet still has it.
- **Soft-displacement, not hard-block**. Once Sarah types, autonomous sheets queue behind her — but idle decay (15 min) eventually relaxes the block so background work isn't starved indefinitely. The on-disk draft makes idle displacement safe.
- **Surface restoration explicitly, never silently**. The Today restore banner shows unfinished captures with Resume / Discard. Auto-restoring a draft Sarah intentionally walked away from would be more confusing than helpful.
- **Plain English in user-facing copy**. No "TTL", no "auto-discard", no "FormDraft". The 7-day notice and dismiss button labels (Close vs Discard) are written so a non-technical user understands them at a glance.
- **Audio retention is gated by capture state, not the other way around**. The WFG retention rule destroys audio after the user confirms derived outputs; an unfinished capture is *derived output not yet confirmed*, so audio destruction defers. The 7-day TTL ensures audio still gets purged eventually if a capture is never finished.
- **Future sheets MUST follow the same pattern** (see context.md §8.8). The arbiter, the soft-displacement guard, the form drafts, the restore banner, the audio gate, and the dismiss semantics form one system; new sheets that bypass any layer reopen the data-loss surface.

**Out of scope** (deliberate, may revisit):
- **iOS companion drafts** — phone-side draft persistence is a separate workstream. Starting a capture on the phone, switching to Mac, will not surface a draft on the Mac.
- **`ContactEnrichmentCoordinator` in-flight approval tracking** — currently the enrichment review sheet is `.userInitiated` priority; the dangling-reference risk in onChange-reload was noted but not directly addressed.
- **Displacement telemetry log** — would help trace future data-loss reports; not built because the primary blocking gates exist.
- **Nested-sheet re-presentation** — when a managed parent sheet is displaced, an unmanaged nested `.sheet` inside it unmounts too. State preservation depends on the nested sheet's own backing (most use `DraftStore`, which is now disk-backed). On re-presentation of the parent, the nested sheet requires an explicit user re-click to reopen — its data survives, its open state does not. Acceptable for now; would require either window-aware `ModalCoordinator` or a "managed nested sheet" abstraction to fix.
- **LinkedIn window cross-sheet conflict** — theoretically possible if two internal sheets fire simultaneously in the LinkedIn import window, but practically gated by the sequential flow.
