...
Add an interactive, force-directed relationship graph to SAM that reveals structural patterns invisible in list and dashboard views: referral network topology, household coverage gaps, recruiting tree health, communication flow direction, and orphaned connections. The graph is a **strategic tool**, not a decoration — every visual element must surface actionable intelligence.
...
4. **Performance-bounded** — Graph layout computation runs off the main thread. Initial layout uses a deterministic algorithm seeded by existing context groupings (households cluster together, recruiting trees form hierarchies). Force simulation refines incrementally without blocking the UI.
...
- Group people by their primary SamContext (household/business). Place context members in a tight cluster.
...
- Tooltip shows: "Hub — connects X people across Y households. Referral conversion rate: Z%"
- Implementation: Standard betweenness centrality algorithm in Swift, O(V × E)

**Overlay 2: Coverage Gap Detection**
- For each household context, check if all members are contacts and if products cover expected needs
- Highlight household clusters with gaps: "Johnson Household: 3 members, only 1 is a client. Sarah (spouse) has no coverage."
...
| SamContext (households, businesses) | G | Household/business edge clusters |
...
**Test**: Create 20 mock people with known contexts, referrals, and evidence links. Assert correct edge count, edge types, node health levels. Assert layout converges (all node velocities < threshold after 300 iterations). Assert layout respects context clustering (household members within radius R of each other).
...
**Test**: Verify hub detection identifies known referral centers in test data. Verify coverage gaps correctly flag households with partial coverage. Verify recruiting tree colors match RecruitingStage data.
