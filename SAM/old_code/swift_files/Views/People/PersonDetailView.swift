...
struct PersonDetailView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                // Note: currently contact is never assigned after loadContactData update,
                // so these sections will not appear until refactored to accept value DTOs or re-enable CNContact fetch on MainActor.
    ...
    private struct ContactLookupResult: Sendable {
        let isValid: Bool
    ...
    private func loadContactData() async {
        ...
        // 1) Validate and collect value bits off the MainActor
        let result: ContactLookupResult = await Task.detached(priority: .userInitiated) { @Sendable in
            let store = ContactsImportCoordinator.contactStore
            // Validate identifier quickly
            guard ContactValidator.isValid(identifier, using: store) else {
                return ContactLookupResult(isValid: false, displayName: nil, contactIdentifier: nil, thumbnailData: nil)
            }
            // Optional: small thumbnail probe (value-only) to avoid a second fetch later
            ...
            let thumb = c?.thumbnailImageData
        ...
        // 2) If invalid, clear state and show banner later
        ...
        // 3) Fetch the full CNContact on MainActor (synchronous call)
        #if canImport(Contacts)
        let store = ContactsImportCoordinator.contactStore
        let fullKeys: [CNKeyDescriptor] = [
        ...
        if let fullContact = try? store.unifiedContact(withIdentifier: identifier, keysToFetch: fullKeys) {
            // Assign full contact for rich sections
            contact = fullContact

            // Build image from either the earlier thumb probe or this full contact
            let data = result.thumbnailData ?? fullContact.thumbnailImageData
            #if os(macOS)
            if let data, let nsImage = NSImage(data: data) {
                contactPhoto = Image(nsImage: nsImage)
            } else {
                contactPhoto = nil
            }
            #else
            if let data, let uiImage = UIImage(data: data) {
                contactPhoto = Image(uiImage: uiImage)
            } else {
                contactPhoto = nil
            }
            #endif
        } else {
            // Couldn’t fetch the full record after validation (rare)
            contact = nil
            contactPhoto = nil
        }
        #else
