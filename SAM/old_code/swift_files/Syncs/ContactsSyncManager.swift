...
final class ContactsSyncManager {
    ...
    // Helper to validate a contact identifier without capturing CNContactStore in a @Sendable closure
    nonisolated private static func validateIdentifier(_ identifier: String, requireSAMGroup: Bool) -> Bool {
        #if canImport(Contacts)
        let store = ContactsImportCoordinator.contactStore
        #if os(macOS)
        if requireSAMGroup {
            let result = ContactValidator.validate(identifier, requireSAMGroup: true, using: store)
            switch result { case .valid: return true; default: return false }
        } else {
    ...
    func validateAllLinkedContacts() async {
        ...
        // Validate each contact using a task group within the current task (no detached sending of closures).
        let reqSAM = requireSAMGroupMembership
        
        // Build a sendable array of task data to avoid capturing validationTasks directly
        let taskData: [(UUID, String)] = validationTasks.map { ($0.0, $0.1) }
        let taskCount = taskData.count
        
        let results: [(UUID, Bool)] = await withTaskGroup(of: (UUID, Bool).self, returning: [(UUID, Bool)].self) { group in
            for index in 0..<taskCount {
                let personID = taskData[index].0
                let identifier = taskData[index].1
                
                // Capture local copies of all variables to ensure they're Sendable
                let reqSAMCopy = reqSAM
                let debugCopy = debugLoggingEnabled
                
                // Note: This task is nonisolated (runs on concurrent thread pool)
                // because it performs potentially slow CNContactStore I/O
                group.addTask { @Sendable in
                    let isValid = Self.validateIdentifier(identifier, requireSAMGroup: reqSAMCopy)
                    if debugCopy {
