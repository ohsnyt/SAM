...
actor GraphBuilderService {
    ...
    func buildGraph(
        ...
        // --- Deduced family edges (deduplicated by exact sourceID+targetID+type) ---
        ...
        for link in deducedFamilyLinks where personIDs.contains(link.personAID) && personIDs.contains(link.personBID) {
            // Exact key: preserves direction so parent/child show as separate arrowed edges
            let key = "\(link.personAID)-\(link.personBID)-\(link.relationType)"
